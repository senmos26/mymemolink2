...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['obtenir'])) {
        ...
        $note = $noteController->getNotesParCode($_POST['codeRecuperation']);
    }
}
...
echo '
    <form action="" method="post">
        ...
        <input type="text" maxlength="10" class="form-control" name="codeRecuperation"><br>
        <button class="btn btn-success" name="obtenir" type="submit">
            Récupérer
        </button>
...
if (isset($note) && is_object($note)) {
    ...
    echo '<button class="btn btn-primary" onclick="copierContenu()">Copier le Contenu</button>';
...
<!-- Ajoutez le script JavaScript en bas de votre fichier PHP -->
<script>
    function copierContenu() {
        // Sélectionner le contenu à copier
        var contenu = document.querySelector('.card-text strong:nth-child(2)').innerText;
        ...
        // Afficher une alerte ou un message pour indiquer que le contenu a été copié
        alert('Contenu copié dans le presse-papiers : ' + contenu);
    }
</script>
